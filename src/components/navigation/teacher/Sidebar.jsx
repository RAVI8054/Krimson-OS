...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { name: "My Classes", path: "/dashboard/teacher/classes", icon: <Users size={20} /> }, // Screen 2
    { name: "Lesson Plans", path: "/dashboard/teacher/lessons", icon: <BookOpen size={20} /> }, // Screen 3
    { name: "Attendance", path: "/dashboard/teacher/attendance", icon: <CalendarCheck size={20} /> }, // Screen 4
    ...
    { name: "Messages", path: "/dashboard/teacher/communication", icon: <MessageSquare size={20} /> }, // Screen 7
    { name: "Calendar", path: "/dashboard/teacher/calendar", icon: <Calendar size={20} /> }, // Screen 8
    ...
    { name: "Reports", path: "/dashboard/teacher/reports", icon: <FileBarChart size={20} /> }, // Screen 11
    { name: "Resources", path: "/dashboard/teacher/resources", icon: <Book size={20} /> }, // Screen 12
    { name: "Reflections", path: "/dashboard/teacher/reflection", icon: <PenTool size={20} /> }, // Screen 13
    { name: "Attd. Summary", path: "/dashboard/teacher/attendance-summary", icon: <PieChart size={20} /> }, // Screen 14
    // Screen 15 is accessed via profile at bottom
  ];

  return (
    <div className="h-screen w-72 p-4 flex flex-col fixed left-0 top-0 z-50">
      {/* Gradient Container - Crimson Tone for Teacher */}
      <div className="h-full w-full rounded-3xl bg-gradient-to-b from-rose-500 via-red-500 to-orange-400 p-4 flex flex-col text-white shadow-2xl relative overflow-hidden">
        
        {/* Decorative Elements */}
        <div className="absolute -top-10 -right-10 w-40 h-40 bg-white opacity-10 rounded-full blur-2xl"></div>
        <div className="absolute bottom-10 -left-10 w-40 h-40 bg-yellow-400 opacity-20 rounded-full blur-2xl"></div>
        ...
        <div className="flex-1 overflow-y-auto space-y-1 relative z-10 custom-scrollbar pr-1">
          {menuItems.map((item, index) => (
            ...
            >
              <span className={({ isActive }) => isActive ? "text-red-500" : "text-white"}>{item.icon}</span>
        ...
        <div className="mt-4 pt-4 border-t border-white/20 relative z-10">
          <div className="flex items-center gap-3 p-3 rounded-xl bg-white/10 backdrop-blur-md hover:bg-white/20 transition-colors">
            <NavLink to="/dashboard/teacher/profile" className="flex items-center gap-3 flex-1">
                <div className="w-8 h-8 rounded-full bg-white text-red-500 flex items-center justify-center font-bold text-xs">T</div>
                <div className="flex-1">
                <p className="text-sm font-semibold">Teacher</p>
                <p className="text-[10px] opacity-80">:: Faculty</p>
                </div>
            </NavLink>
            <button onClick={handleLogout} className="text-white hover:text-red-100 p-1">
                <LogOut size={16} />
            </button>
          </div>
        </div>
