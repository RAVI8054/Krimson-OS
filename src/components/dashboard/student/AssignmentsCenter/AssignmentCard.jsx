    ...
    <div className="bg-white p-6 rounded-3xl shadow-sm hover:shadow-md transition-all border border-slate-100 flex flex-col h-full">
      <div className="flex justify-between items-start mb-4">
        <span
          className={`px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wide border ${
            assign.status.includes("Pending") || assign.status === "In Progress"
      ...
      {tab === "Pending" && (
        <div className="space-y-4">
          <div
            className={`flex items-center gap-3 p-3 rounded-xl border ${getDeadlineColor(assign.daysLeft)}`}
          >
            <Clock size={18} />
          ...
          {assign.status === "In Progress" && (
            <div className="mt-2">
              {scanningId === assign.id ? (
                ...
                <div
                  className={`p-3 border rounded-xl flex items-center gap-2 text-xs font-bold ${scanResults[assign.id].score > 10 ? "bg-red-50 border-red-100 text-red-600" : "bg-green-50 border-green-100 text-green-600"}`}
                >
                  {scanResults[assign.id].score > 10 ? (
                    <XCircle size={14} />
                  ) : (
                    <CheckCircle size={14} />
                  )}
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button className="px-5 py-3 border border-slate-200 rounded-xl text-slate-600 font-bold hover:bg-slate-50 transition-colors">
              Details
            </button>
          </div>
        </div>
          ...
          <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex items-center gap-3">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <p className="text-sm font-bold text-slate-700 truncate">
                {assign.submittedFile || "assignment.pdf"}
              </p>
            </div>
          ...
          {assign.plagiarismFlag && (
            <div className="p-4 bg-red-50 border border-red-100 rounded-2xl">
              ...
              <p className="text-xs text-red-600 font-medium">
                AI detection flagged potential overlap. Plagiarism Score:{" "}
                {assign.plagiarismScore}%
              </p>
            </div>
          )}
      ...
      {tab === "Graded" && masteryData[assign.id] && (
        <div className="space-y-4">
          ...
          <div
            className={`p-4 border-2 rounded-2xl ${getMasteryColor(masteryData[assign.id].score)}`}
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                className={`px-3 py-1 rounded-full text-xs font-bold ${getMasteryBadge(masteryData[assign.id].status)}`}
              >
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-xs font-bold uppercase mb-1 text-slate-600">
                  Next Action
                </p>
                <p
            ...
            {masteryData[assign.id].remedialTasks && (
              <div className="mt-4 space-y-2">
                ...
                {masteryData[assign.id].remedialTasks.map((task, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-2 text-xs text-orange-600 font-medium"
                  >
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-xs text-slate-700 leading-relaxed">
                  {masteryData[assign.id].feedback.wellDone}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-slate-700 leading-relaxed">
                  {masteryData[assign.id].feedback.improve}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-slate-700 leading-relaxed">
                  {masteryData[assign.id].feedback.nextSteps}
                </p>
              </div>
            </div>
          </div>
                ...
                <div className="mt-3 p-3 bg-white rounded-lg border border-slate-200">
                  ...
                  <div className="space-y-1 text-xs text-slate-600">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-1 h-1 rounded-full bg-slate-400"></div>
                      Action:{" "}
                      {masteryData[assign.id].nextAction === "advance"
                        ? "Unlock next assignment"
                        : masteryData[assign.id].nextAction === "reinforcement"
                          ? "Light reinforcement required"
                          : masteryData[assign.id].nextAction === "remedial"
                            ? "Targeted remediation"
                            : "Full remediation + teacher notification"}
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-1 h-1 rounded-full bg-slate-400"></div>
                      Progress status:{" "}
