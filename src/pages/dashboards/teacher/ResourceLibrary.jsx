    ...
    <div className="space-y-8">
      <div className="bg-white p-8 rounded-3xl shadow-sm flex flex-col md:flex-row gap-6 items-center justify-between">
         <div className="flex-1 w-full relative">
            ...
            <input type="text" placeholder="Search approved materials..." className="w-full pl-12 pr-4 py-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 focus:ring-rose-100 transition-all font-medium text-slate-700" />
         </div>
         <button className="bg-rose-600 text-white px-8 py-4 rounded-2xl font-bold shadow-lg hover:bg-rose-700 flex items-center gap-3 transition-transform hover:scale-105 active:scale-95">
            ...
            <div key={i} className="bg-white p-6 rounded-3xl shadow-sm hover:shadow-md transition-all cursor-pointer group">
               <div className="flex justify-between items-start mb-6">
                  <div className={`p-4 rounded-2xl ${
                     ...
                     i===2 ? 'bg-green-50 text-green-600' : 'bg-orange-50 text-orange-600'
                  }`}>
                     <FileText size={24} />
                  </div>
                  <span className="bg-slate-100 text-slate-500 text-[10px] font-bold px-2 py-1 rounded">24 Files</span>
               </div>
               <h4 className="font-bold text-slate-800 text-xl group-hover:text-rose-600 transition-colors">{subj}</h4>
