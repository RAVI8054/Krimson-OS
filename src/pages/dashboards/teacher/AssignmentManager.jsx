    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
         {assignments.map((assign) => (
           <div key={assign.id} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 hover:shadow-md transition-all">
              <div className="flex justify-between items-start mb-4">
                 <div className="p-3 bg-blue-50 text-blue-600 rounded-xl"><CheckSquare size={24}/></div>
                 <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${
                    assign.due === 'Today' ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'
                 }`}>Due {assign.due}</span>
              </div>
              ...
              <div>
                 ...
                 <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden mb-4">
                    <div className="bg-blue-500 h-full rounded-full" style={{ width: `${(assign.submitted/assign.total)*100}%` }}></div>
                 </div>
              </div>
              ...
              <div className="flex gap-2 mt-4">
                 ...
                 <button className="flex-1 py-2 bg-slate-800 text-white rounded-xl text-xs font-bold hover:bg-slate-900">Grade Now</button>
              </div>
           </div>
         ))}
      </div>
