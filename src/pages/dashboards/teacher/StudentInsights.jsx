  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         <div className="bg-gradient-to-br from-green-400 to-emerald-600 rounded-3xl p-8 text-white shadow-lg relative overflow-hidden">
             <div className="relative z-10">
                <p className="text-green-100 font-bold uppercase text-xs">Top Performing Class</p>
         ...
         <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 flex items-center justify-between">
             <div>
                ...
                <p className="text-green-500 font-bold text-sm mt-1">vs Last Month</p>
             </div>
      ...
      <div className="bg-white rounded-3xl p-8 shadow-sm">
         <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2">
            <AlertOctagon className="text-red-500" /> Students Needing Attention
         </h3>
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {insights.riskStudents.map((stu, i) => (
              <div key={i} className="border border-red-100 bg-red-50/30 rounded-2xl p-6 relative group hover:shadow-md transition-shadow">
                 <div className="flex items-center gap-4 mb-4">
                    <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center text-red-500 font-bold shadow-sm text-lg">
                       {stu.name.charAt(0)}
                    </div>
                    <div>
                       <h4 className="font-bold text-slate-800">{stu.name}</h4>
                       <p className="text-xs text-red-500 font-bold uppercase tracking-wider">High Risk</p>
                    </div>
                 </div>
                 <p className="text-sm text-slate-600 mb-4">Flagged for: <span className="font-bold">{stu.issue}</span></p>
                 <button className="w-full py-2 bg-white border border-red-200 text-red-600 rounded-xl text-xs font-bold hover:bg-red-50">
                    Send to Counselor
                 </button>
              </div>
            ))}
